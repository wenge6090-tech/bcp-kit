...
- gate↔evolve 字段契约是 §12.1 `{domain, file}`：memory-gate 域注入须写 `{domain, file}`（kind/target 为兼容字段），evolve 只统计 `kind=domain` 且兼容读 target——改任一端必须同步对端，错位 = 域统计全为 `?` 假域、死重误判、§12.6 冷启动保护失效。
