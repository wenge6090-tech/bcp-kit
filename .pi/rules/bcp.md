...
> memory-gate 首触 `bcp/`、`.pi/extensions/` 时注入。格式：模式页（问题+根因+证据+规避句，Blueprint §7.3）。
>
> **目录**（看此判断相关性，无需整读）：
...
> 8. 机械件改动无冒烟 = 静默死亡——异常被吞/分支未跑即潜伏 → 改 gate 跑契约向量+真件冒烟，改 evolve 跑 evolve-smoke
> 9. 账本多态 mode 按 mode 取值——裁决记录无 commits = 基线静默清零 → 按字段判别
> 10. 计划机械约束三条——interfaces 扫描面 / yaml 围栏 / excludes 豁免 → 见条目
> 11. accept 否定断言用通用词误伤——合法简介含同词即假 FAIL → 先 grep 目标文件现况
> 12. 重排编号后裸 § 引用静默漂移——号存在 ≠ 指对 → 旧号→新号映射逐条改 + 脚本复扫

## 模式 1：gate↔evolve 字段契约错位

- **问题**：只改 memory-gate 或 evolve 单端的域注入字段，域统计全为 `?` 假域、死重误判、Blueprint §5.7 冷启动保护失效。
- **根因**：两件各自消费对方写入的字段（gate 按 Blueprint §5.2 写 `{domain, file}`，evolve 只统计 `kind=domain` 且兼容读 `target`），中间无 schema 校验层，错位是静默的。
...
- **根因**：append-only 是 Blueprint §5.2 法律，测试写入会永久混入 evolve 生产统计。
...
- **问题**：① memory-gate 两处缺陷长期潜伏无人知觉：域注入读规则用游离变量 `cwd`，ReferenceError 被 catch 吞成 fail-open——闸门①实际已死；skill-recall 正则无捕获组，JS `dm[1]`=undefined → target 静默丢失，evolve 统计成 `?` 假域。TypeScript 能加载 ≠ 行为正确；② `bcp/evolve.py` 的预填确认率聚合引入后一次都没跑过（只在 sleep 对账时被调用），无 judged 记录时 `by_skill` 未绑定 → UnboundLocalError，退出码 1、REPORT 未落账、巡检无法醒来。
- **根因**：fail-open 律是双刃剑——不死锁的代价是一切异常都伪装成「规则缺失放行」；且召回事件从未发生过（召回=0）、judged 记录从未有过，缺陷无自然触发机会；报告器无自动入口也无测试面。
- **证据**：2026-09-11 宿主契约抽取任务（契约向量集一跑即抓出 gate 两缺陷）；同日 sleep 对账实测（by_skill 崩溃，潜伏 3 小时——该聚合提交 08:52，上一次成功运行 07:20）。
- **规避句**：改 memory-gate 后必跑 `python3 kit/host-contract.test.py` + `node .pi/gate-smoke.mjs`（真件导入 + mkdtemp 沙盒，真账本不触碰）；改 `bcp/evolve.py` 后必跑 `python3 kit/evolve-smoke.test.py`（四种账本形态 + 真账本隔离断言）；两件均不许只信「能加载/能 import」——**新加的分支必须真跑过一次**，没跑过的代码等于没写。
...
## 模式 11：accept 否定断言用通用词误伤

- **问题**：`! grep -q 某词 目标文件` 断言「某内容已删」，但该词同时出现在合法残留（组件简介、术语行）——对碰假 FAIL，被迫改计划重跑。
- **根因**：断言词取自被删段落的主题词，未核查同词在文件其他位置的合法出现。
- **证据**：2026-09-11 文档分层任务连续两轮：删「对碰税」论证段但术语在适用边界句合法残留；删「晋升」引注但 evolve 简介含「晋升/降级候选」。
- **规避句**：写否定断言前先 grep 该词在目标文件的现况，选仅存于被删内容的专名短语（如「显式支付的维护代价」「书记员」）；或断言新增结构而非删除主题词。

## 模式 12：文档重排编号后裸 § 引用静默漂移

- **问题**：文档章节重排后，机械件注释里的裸 `§x.y`（未点名文档，隐含 Blueprint.md）指向同号异义的旧章节或悬空号；悬空扫描只抓「号不存在」，号存在但含义已变的一律漏网。
- **根因**：重排时只批量改了「点名文档」的引用（README §/Blueprint §），裸引用与语义映射之间无校验层；§ 号复用（新 §4.2 是别的内容）使错误不可见。
- **证据**：2026-09-11 文档分层收口任务——README→Blueprint 迁移后 28 处裸 § 漂移（§4.2/§4.3/§4.5/§4.6/§4.7/§9.1/§9.2 全部指错），全仓悬空扫描报 0；人工审查才发现。
- **规避句**：重排编号必须建「旧号→新号」映射表逐条改裸引用；收尾用脚本复扫（正则 `§(\d+(?:\.\d+)*)`，未点名文档者按 Blueprint 校验 `_headings()`）——**号存在 ≠ 指对**，语义匹配须人工复核。含「点名文档」行的裸引用不在此扫描面，删改点名行时注意别绕过。
