...
范式 spec：`BCP.md`（§11 记忆分层）。项目索引与跨域硬约束：`AGENTS.md`（自动加载）；按域避坑细则在 `.pi/rules/*.md`，由 memory-gate 扩展在首次触碰对应代码域时自动拦截注入——预期行为，吸收后重试同一调用即可，非报错。
流程：设计定论文档（`Blueprint.md`，项目可选）→ 计划（瞬态）→ 代码 → `/check` 机械检查 → 经验沉淀回 `AGENTS.md` / `Blueprint.md`。
...
- 用户确认后：先把新定论写回 `Blueprint.md` 对应章节，再走 /plan。
...
- 计划头 `mode` 字段（explore | infer，缺省 infer）：产出原始轨迹 / `deliverables/SKILL.md` / 技能进化类任务必须 `explore`——元组装裸跑（不读储层、不召回案例），R7 机械裁决（BCP §3.1）。
...
- 0–1 条设计定论 → `Blueprint.md`（范式层 → `BCP.md`，需用户批准）。
- 删除已完成任务的计划文件；失败任务的计划连同裁决留档，供换会话续跑。
- 失败典藏：FAIL 任务强制压缩一条失败模式（标题+规避句 ≤200 字符）→ 追加 `bcp/ledger.jsonl`（mode=failure）+ 对应 `.pi/rules/<域>.md`；只回注模式，不回注尸体（BCP §1）。
- 技能沉淀默认文本轨道 `deliverables/SKILL.md`；Python 编译仅显式 `--compile-python` 或验证集成功率 ≥95% 触发（BCP §3.2）。
