...
> 本仓库 = BCP 范式工具包（模板），自身亦按该范式运作（自举验证）。范式 spec：`BCP.md`；工作流：`.pi/APPEND_SYSTEM.md`；使用说明：`README.md`。
...
- 工具包件（BCP.md/plan.md/bcp//.pi//kit/）变更 = 范式变更：先在会话中给用户看 diff 概览再落，范式层定论需人批准。
