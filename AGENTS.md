...
> 本仓库 = BCP（Blueprint Completion Protocol，蓝图完形协议）范式工具包（模板），自身亦按该范式运作（自举验证）。范式 spec + 设计：`Blueprint.md`；工作流：`.pi/APPEND_SYSTEM.md`；门面：`README.md`（英文）/ `README.zh-CN.md`（中文）。

## 结构

| 件 | 本质 |
|----|------|
| `README.md` / `README.zh-CN.md` | 门面 + 导言（英文主门面 / 中文版；液态，外部受众，变更无垒） |
