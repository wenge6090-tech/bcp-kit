...
> **范式定论**。开发 = 人类裁决设计（B）→ 符号组装上下文（P）→ 阳完形产出代码（C）→ 阴·机械对碰守接缝 → 固化回流。
> LLM 是条件生成器（词语接龙与完形填空），BCP 把开发组织成「补全」而非「创作」：计划定义空格形状，模型填空。
>
> 配套工件：转化协议 `plan.md`（P 的 schema）· 工作流内核 `.pi/APPEND_SYSTEM.md`（A 的注入样本）· 机械对碰器 `bcp/check.py` + `bcp/bcp.toml`（阴）· 实现事实 `AGENTS.md`。
>
> **宿主无关**：本 spec 与 `bcp/` 机械件只依赖 Python 3.11+ 标准库；`.pi/` 件为 pi 宿主适配层（换宿主只需重写该层，§9 最小契约）。历史定论沿革（V80–V83）详见 taiji 仓库 `Blueprint.md`（冻结档案）。

---

## 0. 哲学基础

1. **态射分段可检**。代码语言是自然语言的态射，但意图→代码的直接映射不是函数（一对多）。BCP 插入中间对象把全局映射拆成三段局部映射：`意图 →B→ P →C`，逐段校验交换性。全局无损不可直接校验；**分段可检且误差逐段可归因 = 接近无损的可执行定义**。
...
| A 规则内核 | **液**（储层） | agent | 持续形变（汇流/晋升/降级）；按需倾倒；受墙约束 |
| B 蓝图 | **固**（晶体） | 人类 | 结晶需人类批准；按需检索，不常驻 |
...
| 乐章 | 相变 | 职责 | LLM 触点 |
|---|---|---|---|
| 元组装 | 熔化+汽化 | 按 §3.1 注入开关读 B 相关章节 + A 倾倒样本 + 代码索引对账 → 产出 P | 起草 P 内容（schema 由协议定，模型填空不定义空格） |
| 阳完形 | 气态执行 | 消费 P + A 内核产出代码，accept 即空格填满 | **唯一概率相位之二：完形 C** |
| 阴对碰 | 液化 | 机械对碰产出裁决与账本记录 | **零 LLM**（见 §4） |
| 固化回流 | 固化+汇流 | 避坑→A（无垒）、定论→B（需批准）、事实→C | 零 LLM（候选可由 LLM 起草，采纳是人的） |
...
- **默认产物双轨**：阳产出 `deliverables/SKILL.md`（纯文本规则 + Frontmatter 元数据）+ `deliverables/declaration.yaml`（结构化自声明，凭证协议不变）。
- **编译降级为可选优化**：Python 固化仅在 ① 人类显式 `--compile-python`，或 ② 文本规则验证集成功率 ≥95% 时触发一次；其余场景文本即技能。
- **与蓝图定论⑥不冲突**：文本技能走 prompt 注入轨道（LlmJudgement 家族），不是机械执行体——「执行体统一 Python」继续成立；运行时 `SkillKind::Text` 为候选扩展（§10 待做）。
...
- **门控顺序（双保险，符号阀前置）**：① 经验阀——验证集分数上涨只产生**采纳候选**；② 符号阀——`check.py --collision` 的引用悬空 / 注入风险 / 路径逃逸**一票否决**，验证集满分不抵消符号 FAIL。统计分数进不了判据表（蓝图定论⑬），只能决定候选优先级——软分数可以提效，不可以越权。
...
- **回流协议**：任务终态固定产出 ≤3 条避坑候选（入 A，无垒）+ 0–1 条定论候选（入 B，需批准）。FAIL 任务额外强制产出一条失败模式（mode=failure 入账本 + 避坑入 A，见 §1）。可复用的成功轨迹按 §3.2 双轨沉淀，编译不默认。回流是循环的固定相变，不是「记得做的美德」。
...
7. **域错配**——给自带裁判器的域上重机制（编码域自带编译器/测试/机械检查，元可外包给人、阴可外包给工具链，单 LLM 循环即够）。免疫：最小架构是域的函数——三相循环仅在「人不在场 + 无现成裁判」的开放域成立（§13）。
...
- 本仓库（范式模板）：`bcp/` 对碰器（R5 引用完整性 / R6 计划对碰 / R7 探索纯净开箱即用；R1–R4 项目专属按需配置）；`.pi/` 工作流内核四步化 + memory-gate 三闸门（域规则 / 大文件附注 / compaction 恢复）+ pre-commit 提交闸门。

## 12. 演化算子落地（统一证据流 + evolve.py）

> 设计动因：§7 的晋升/降级算子原本无消费者。定论：证据流统一进 `bcp/ledger.jsonl`（append-only），新增零 LLM 报告器产出候选数据，裁决权在人。
...
### 12.2 evolve.py（零 LLM，纯标准库，守 §9 移植契约）

- 输入：`bcp/ledger.jsonl` + check.py 源中的规则名注册表（正则机械提取 seam 表键，不建副本）+ `.pi/rules/` 清单。
- 输出：纯文本报告——① 各 check 规则全历史/窗口内命中数与末次命中；② 各域 gate 注入计数；③ 降级候选（窗口内零命中且低频高害存疑者，只列数据）；④ 从未注入的规则文件（死重候选；冷启动期改列「待积累」，见 §12.6）；⑤ 高频 FAIL 规则（强化证据）。
...
- **降级**：报告给出零命中/死重候选 → **必经人裁决**（「未防过事故」不可机械测；零命中的低频高害规则如 R1/R2/R4 不宜自动降）→ 人批准后迁 BCP.md 叙事或删除。
...
### 12.6 冷启动语义（gate 证据无效期）

> 设计动因：管道刚落地时账本 0 条 gate 事件，④ 节把全部规则文件误标「死重候选」（2026-09-07 首份报告实证，5 文件全列）。定论：死重判定必须有管道活性证据。

- **冷启动判定**：`sum(gate_by_domain.values()) == 0`（账本中无任何 gate 事件）。
...
## 11. A 内核记忆分层（四层，宿主侧落地）

> 设计动因：pi 无跨会话记忆，上下文文件即记忆。AGENTS.md 全量 42KB 常驻违反 §6 墙纪律；嵌套文件+自觉 read = 纪律依赖反模式（§8.5）。定论：规则按可靠性分四层，每条规则必须落一层，不允许无主散文。

### 11.1 四层（按可靠性降序）

| 层 | 机制 | 触发方式 | 腐烂面 |
|---|---|---|---|
| 1 凝结 | 规则 → check.py R 规则 / cargo test | 违反即红，零阅读依赖 | 无（跟代码走） |
...
| 4 核心索引 | AGENTS.md 只留环境信息 + L0-L7 路径索引 + 类型索引 + 跨域硬约束，≤6KB | 每会话常驻 | 路径由 R5 守护 |
...
flowchart LR
    ...
    subgraph L1["层1 机械守护（check.py R1-R6 + cargo test）"]
        MECH["R1-R6 终局验证"]
    end
...
### 11.4 分流审计（案例：taiji 实例原 AGENTS.md 24 节 → 去处）
...
- 拦截仅作用于 read/edit/write 三工具的 path 参数；bash 是已知缺口（改代码主路径是 edit/write，残余风险归 §17 既有坏习惯条款）。
...
- **范式真源在宿主**：本 spec / A 内核 / 对碰器的运行载体是通用 agent 宿主（提示词 + 零 LLM 脚本）。改范式 = Markdown diff，下次会话生效，成本近零、行为可预期。专用内核跑范式 = 自带状态重建 + 每轮裁决的 token 税，属 §8.7 域错配。
