...
四个对象沿链相变：**B 蓝图**（固，人类裁决）→ **P 计划**（瞬态，agent 消费）→ **C 代码**（固，唯一实现事实）→ **A 规则内核**（液，回流沉淀）。两个守门相位：**阳完形**（LLM 填空，唯一概率相位）与**阴对碰**（零 LLM 机械裁决）。
...
- **与蓝图定论⑥不冲突**：文本技能走 prompt 注入轨道（LlmJudgement 家族），不是机械执行体——「执行体统一 Python」继续成立；运行时 `SkillKind::Text` 为候选扩展（§8.2 待做）。
...
flowchart LR
    ...
    subgraph L1["层1 机械守护（check.py R1-R7 + cargo test）"]
        MECH["R1-R7 终局验证"]
    end
