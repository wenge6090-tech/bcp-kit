...
> 机械校验：`./bcp/check.py --plan <计划文件> --collision`。范式背景见 `BCP.md`。
...
```yaml
goal: 一句话说清要做什么
mode: infer                   # 可选：explore | infer（缺省 infer）；产出 deliverables/SKILL.md 或 skill_evolution 任务必须 explore（R7，BCP §2.3）
items:
  - id: P1
    blueprint: §5.2            # 必填：设计章节锚 `§x.y`（默认 Blueprint.md）或 `文件§x.y`（如 BCP.md§2.3）；悬空 = FAIL
...
3. 用户确认计划后才进入实现。
4. 技能进化 / 原始轨迹采集任务必须 `mode: explore`（元相位裸跑，禁先验注入，BCP §2.3）；可复用成功轨迹按 BCP §2.4 双轨沉淀（文本技能一等公民，编译不默认）。
...
- 完成后：删除计划文件；避坑沉淀到 `AGENTS.md`，设计定论回写 `Blueprint.md`（需用户批准）。
