...
四个工件沿链相变：**B 设计**（固，元批准）→ **P 计划**（气态内容物，agent 消费）→ **C 代码**（固，唯一实现事实）→ **A 规则**（液，回流沉淀）。两个关键相位：**阳完形**（LLM 填空，与组装并列的两个概率相位之一）与**阴对碰**（零 LLM 机械裁决）。
...
| **固** | 窗口外，检索式 | 仓库事实 | 代码、设计文档、账本 | 只检索所需片段，永不整体进窗口 |

一轮任务就是一轮完整相变：**组装**（熔化固体的设计章节 + 倾倒液体的规则 → 气态计划）→ **阳完形**（气态计划 + 规则 → 固态代码）→ **阴对碰**（机械裁决 → 液态记录入账本）→ **固化回流**（值得留的压回液或固：避坑入规则、设计入文档、事实入代码）。
...
> 证据（WikiSkill, Tang et al. 2026 主结果）：纯文本 Markdown 规则平均 +17~24% 且跨模型迁移好；强制编译 Python 的工程成本（编译耗时 / 冒烟压测）在绝大多数场景 ROI 为负。
...
- **裁决落账**：晋升获批 / 被拒后追加一条 ledger 记录 `{mode:"evolve", verdict:"PROMOTED"|"REJECTED", target:"<规则名>", source:"<来源失败模式标题，可选>", note:"<原因>"}`——append-only 下以此实现失败模式→规则的溯源（不回填历史记录），evolve ⑥ 节回放，被拒候选不重复提案（WikiSkill skill-impact 同构）。
