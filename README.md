...
- **门控顺序（双保险，符号阀前置）**：① 经验阀——验证集分数上涨只产生**采纳候选**；② 符号阀——`check.py --collision` 的引用悬空类机械检查（模板开箱 R5–R7；注入风险 / 路径逃逸等为项目专属，按 §5.4 案例凝结进 R1–R4）**一票否决**，验证集满分不抵消符号 FAIL。统计分数进不了判据表，只能决定候选优先级——软分数可以提效，不可以越权。
